	...
	for (i = 0; i <= 7; i++)
	{
		for (b = 0; b <= 3; b++)
		{
			...
			if (i % 2 == 0)
			{
				...
				else if (i <= 7) mass[i][(2 * b) + 1].col = 0;
			}
		}
		b = 0;
	}
...
int check_all_kills() //check can all checkers kill anyone
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		{	
			if (mass[k][i].col!=0)
			{
				if ((k + 2 < 8) || (k + 2 > 0) || (i + 2 > 0) || (i + 2 < 8) || (mass[k][i].col = hod)) a = a + check_kill(&(mass[k][i]), &(mass[k + 2][i + 2]));
				if ((k + 2 < 8) || (k + 2 > 0) || (i - 2 > 0) || (i - 2 < 8) || (mass[k][i].col = hod)) a = a + check_kill(&(mass[k][i]), &(mass[k + 2][i - 2]));
				if ((k - 2 < 8) || (k - 2 > 0) || (i + 2 > 0) || (i + 2 < 8) || (mass[k][i].col = hod)) a = a + check_kill(&(mass[k][i]), &(mass[k - 2][i + 2]));
				if ((k - 2 < 8) || (k - 2 > 0) || (i - 2 > 0) || (i - 2 < 8) || (mass[k][i].col = hod)) a = a + check_kill(&(mass[k][i]), &(mass[k - 2][i - 2]));
			}
...
void check_hod(square* sq)
{
	...
	if ((*sq).col == 1)
		{
		if ((posy > 7)||(posx+1>7)||(posx-1<0)) return;
		else if ((mass[posy][posx + 1].col != 0) && (mass[posy][posx - 1].col != 0)) return;
		else if (mass[posy][posx + 1].col == 0)
			{
				mass[posy][posx + 1].move = 1;
				mass[posy][posx + 1].moved = sq;
			}
		else if (mass[posy][posx - 1].col == 0)
			{
				mass[posy][posx - 1].move = 1;
	...
	if ((*sq).col == 2)
		...
		if ((mass[posy][posx + 1].col != 0) && (mass[posy][posx - 1].col != 0)) return;
		else if ((mass[posy][posx + 1].col != 0) && (mass[posy][posx - 1].col != 0)) return;
		else if (mass[posy][posx + 1].col == 0)
			{
			mass[posy][posx + 1].move = 1;
